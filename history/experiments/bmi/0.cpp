...
void setup()
{
    ...
    // Scan for I2C devices to debug address issues
    ...
    // Address is optional (defaults to 0x68) - Use Wire1 for Nano 33 BLE
    int8_t result = imu.beginI2C(i2cAddress, Wire1);
    if(result != BMI2_OK)
    {
        Serial.println("Error: BMI270 not connected with primary address!");
        Serial.print("Trying primary address: 0x");
        Serial.println(i2cAddress, HEX);
        
        // Try secondary address
        i2cAddress = BMI2_I2C_SEC_ADDR; // 0x69
        Serial.print("Trying secondary address: 0x");
        Serial.println(i2cAddress, HEX);
        
        result = imu.beginI2C(i2cAddress, Wire1);
        if(result != BMI2_OK)
        {
            Serial.println("Error: BMI270 not found on either I2C address!");
            while(1) 
            {
                Serial.println("BMI270 initialization failed. Check connections and I2C addresses.");
                delay(5000);
            }
        }
    }
