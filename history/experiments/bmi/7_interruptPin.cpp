...
const float ACCEL_THRESHOLD = 3.0;  // Maximum g difference allowed
const float GYRO_THRESHOLD = 100.0; // Maximum deg/sec difference allowed
...
void loop()
{
    ...
    if(interruptOccurred)
    {
        ...
        if(interruptStatus & BMI2_FWM_INT_STATUS_MASK)
        {
            ...
            {
                // Check if current reading is too different from previous
                bool shouldPrint = true;
                
                if(hasPreviousValues) {
                    // Calculate differences for acceleration
                    float accelXDiff = abs(fifoData[i].accelX - prevAccelX);
                    float accelYDiff = abs(fifoData[i].accelY - prevAccelY);
                    float accelZDiff = abs(fifoData[i].accelZ - prevAccelZ);
                    
                    // Calculate differences for gyroscope
                    float gyroXDiff = abs(fifoData[i].gyroX - prevGyroX);
                    float gyroYDiff = abs(fifoData[i].gyroY - prevGyroY);
                    float gyroZDiff = abs(fifoData[i].gyroZ - prevGyroZ);
                    
                    // Drop reading if any value changed too much
                    if(accelXDiff > ACCEL_THRESHOLD || accelYDiff > ACCEL_THRESHOLD || accelZDiff > ACCEL_THRESHOLD ||
                       gyroXDiff > GYRO_THRESHOLD || gyroYDiff > GYRO_THRESHOLD || gyroZDiff > GYRO_THRESHOLD) {
                        shouldPrint = false;
                    }
                }
                ...
                if(shouldPrint) {
                    ...
                    Serial.print("\t");
                    Serial.print(fifoData[i].sensorTimeMillis);
                    ...
                    prevAccelX = fifoData[i].accelX;
                    prevAccelY = fifoData[i].accelY;
                    prevAccelZ = fifoData[i].accelZ;
                    prevGyroX = fifoData[i].gyroX;
                    prevGyroY = fifoData[i].gyroY;
                    prevGyroZ = fifoData[i].gyroZ;
